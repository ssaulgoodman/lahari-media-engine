...
const stepToParam = (step: AppStep): string => {
  if (step === AppStep.UPLOAD) return 'queue';
...
const TAB_PROJECT_KEY = 'lahari:tab:projectId';
const TAB_STEP_KEY = 'lahari:tab:step';
const GLOBAL_PROJECT_KEY = 'lahari:projectId';
const GLOBAL_STEP_KEY = 'lahari:step';
...
export const usePersistedProject = ({
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      ...
      const globalId = localStorage.getItem(GLOBAL_PROJECT_KEY);
      const globalStep = validStoredStep(localStorage.getItem(GLOBAL_STEP_KEY));
