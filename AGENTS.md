...
- `script.md` is the editable script working artifact. Agents should make surgical file edits there, then call `run_action(apply_script)` with markdown; the action parses the draft, checks `scriptFingerprint` drift, validates references/durations, and applies through the atomic script RPC.
