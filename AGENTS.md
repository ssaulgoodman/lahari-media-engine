...
Artists do not use this repo. They install the Codex plugin or direct Claude Code MCP connection from deployed Mirage `/connect`, authenticate OAuth-first when the client supports it, restart their harness, open any empty folder, and ask to open a project. For project-file sync, a logged-in Mirage CLI can run `mirage sync <projectId>`; use `mirage sync <projectId> --state` for fast read-only snapshots and `mirage sync <projectId> --payloads` for prompt receipts only. Otherwise the agent uses `mint_cli_token` for a short-lived one-off sync. Bearer-token install snippets remain fallback for clients without MCP OAuth. On Windows/Codex, prefer installing the CLI once so `npx` is not downloading code while holding a live token. No engine code on the artist's machine.
...
- `mirage/projects/<projectId>/state/` is overwritten from Supabase and should not be hand-edited. `state/payloads/` contains latest per-shot composed storyboard/video payloads for local debug after sync; use live `describe_prompt`/dry-run when you need current server truth without syncing. Routine agent freshness should usually be `mirage sync <projectId> --state`; full sync is for editable drafts/config/handoff refresh.
...
- Storyboard action bridge: prefer `run_action` for `apply_storyboard_prompts`, `import_storyboard_image`, `lock_storyboard`, and `unlock_storyboard`. For local/native storyboard PNGs created by Codex imagegen, POST to `/api/agent/uploads` with `purpose=storyboard_image`, then call `import_storyboard_image({ shotId, sourceAssetId, lock: true })` to attach and approve that exact board. For paid storyboard generation/refine, use `run_action(generate_storyboard, { dryRun: true })` to inspect exact prompt/refs/params before spend when context matters, then `start_job` with `generate_storyboard` or `refine_storyboard_image` after artist approval. `bulk_generate_storyboards` remains hidden from the materialized agent surface until proper async batch fan-out exists. Use `contextOverrides` on storyboard generation when the agent needs a one-off ref bundle rather than the shot's default style/cast/env/previous-board refs.
- Native worker pattern: use Codex/Claude native subagents or background workers for bounded off-thread work when it protects the main director context: imagegen repairs, visual triage, continuity checks, prompt-payload audits, or export packaging. Mirage does not create, coordinate, or persist those workers. Name the worker role in the packet (`Image Repair Worker`, `Video Payload Auditor`, `Continuity Auditor`, etc.) and use the packet/receipt contract in `docs/agent-working-method.md`; workers may inspect, draft, repair local media, or package evidence, but the main director stays responsible for Mirage truth reads, artist approval, paid actions, uploads/imports, locks, and all Mirage state writes.
...
Remote artist notebooks use the two-tier layout: shared Mirage files at the workspace root, project files under `mirage/projects/<projectId>/`. The MCP connection is OAuth-first where the harness supports it. For file sync, use logged-in `mirage sync <projectId>` when available; `--state` refreshes read-only snapshots/payloads quickly, `--payloads` refreshes composed prompt receipts only, and full sync refreshes editable drafts/config/handoff files. Otherwise use `mint_cli_token` plus the returned Mirage CLI command as the one-off path, and `write_project_notebook` as the heavy MCP fallback for no-shell harnesses.
