...
Timeline editor features include media library, video uploads, extra generated clips, split-at-playhead, ripple delete, horizontal scroll, version append, and render history. Treat timeline drafts as sacred: regenerated shot clips should appear as new media-library takes, not overwrite the artist's saved timeline. The render Media Library has a sidebar entry point, badges/new markers for clips not yet in the timeline, upload-to-library for external video clips, create-extra for B-roll/insert clips that do not mutate canonical script shots, soft-hide for unwanted takes, and version append into the timeline. Sync renderer timeline copies with `cd remotion-renderer && npm run sync-timeline` after changing upstream timeline composition code.
