...
- Native worker pattern: use Codex/Claude native subagents or background workers for bounded off-thread work when it protects the main director context: imagegen repairs, visual triage, continuity checks, prompt-payload audits, or export packaging. Mirage does not coordinate those workers. Use the packet/receipt contract in `docs/agent-working-method.md`: the worker may inspect, draft, repair local media, or package evidence, but the main director stays responsible for Mirage truth reads, artist approval, paid actions, uploads/imports, locks, and all persisted state writes.
