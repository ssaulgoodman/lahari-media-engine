...
- `mirage/projects/<projectId>/state/` is overwritten from Supabase and should not be hand-edited. `state/payloads/` contains latest per-shot composed storyboard/video payloads for local debug after sync; use live `describe_prompt`/dry-run when you need current server truth without syncing.
