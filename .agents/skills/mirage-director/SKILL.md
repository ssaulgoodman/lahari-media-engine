...
- For style image candidates, use `run_action({ actionKey: 'generate_style_candidates', input: { note?, promptOverride?, guideAssetId? } })`. Lock a generated or uploaded style asset with `run_action({ actionKey: 'apply_style_direction', input: { style: { sourceAssetId, styleDescription? } } })`.
...
- For Audio work, prefer `generate_dialogue_audio`, `apply_audio_plan`, and `apply_cast_voice` through `run_action`. Use `generate_dialogue_audio` with `dryRun: true` for TTS cost/missing voices before spending. Overlay TTS needs cast voices; lipsync video generation does not.
