...
const help = () => `Lahari CLI ${pkg.version}

Usage:
  lahari sync <projectId> [--cwd <dir>] [--force] [--api-url <url>]

Environment:
  LAHARI_CLI_TOKEN  Short-lived token from Lahari MCP mint_cli_token
  LAHARI_TOKEN      Alias for LAHARI_CLI_TOKEN
  LAHARI_API_URL    Defaults to ${DEFAULT_API_URL}
`;
...
const postNotebookSync = async (opts, knownHashes) => {
  const bearer = token();
  if (!bearer) {
    fail('auth_missing', 'Set LAHARI_CLI_TOKEN from the Lahari MCP mint_cli_token tool before running sync.');
  }
  const response = await fetch(`${apiUrl(opts)}/api/notebook-sync/projects/${encodeURIComponent(opts.projectId)}/notebook`, {
    ...
    headers: {
      ...
      'x-lahari-cli-version': pkg.version,
    },
...
const sync = async (opts) => {
  ...
  writeText(statePath, `${JSON.stringify({
    kind: 'lahari.notebook.sync_state',
