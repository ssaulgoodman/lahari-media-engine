...
- Video: use `run_action(generate_video, dryRun: true)` for requirements/cost, then `start_job(generate_video)` after approval. If a prior video attempt says provider outcome is unknown or pending, retry only after the artist accepts `acknowledgePreviousChargeRisk: true`.
