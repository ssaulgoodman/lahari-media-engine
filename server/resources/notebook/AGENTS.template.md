...
- Video: use `run_action(generate_video, { dryRun: true })` for requirements/cost and prompt composition, then `start_job(generate_video)` after approval. In storyboard mode, dry-run returns the exact composed prompt with `format`, `animation`, `beat`, `refs`, `cut_plan`, `audio`, and `guardrail` segments plus source/edit paths. HF music-video excludes `beat` by default; pass `contextOverrides.includeShotBeat=true` only when the shot direction should be sent once, or persist `videoPromptSlots.includeShotBeat=true` through `apply_shot_prompts` when future Studio/agent regenerations should keep sending it.
