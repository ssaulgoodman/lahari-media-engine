...
- Video: use `run_action(generate_video, { dryRun: true })` for requirements/cost and prompt composition, then `start_job(generate_video)` after approval. In storyboard mode, dry-run returns the exact composed prompt with `format`, `animation`, `beat`, `refs`, `cut_plan`, `audio`, and `guardrail` segments plus source/edit paths.
...
- Use `contextOverrides` for one call's input bundle: include an extra guide/ref, exclude a wrong ref, drop the style image, include previous storyboard, change style-note sections, or for video drop slots such as `includeShotBeat: false` / `includeCutPlan: false`.
