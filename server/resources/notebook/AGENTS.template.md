...
- Video: use `run_action(generate_video, { dryRun: true, dryRunSummary: true })` for routine requirements/cost and prompt anatomy, then `start_job(generate_video)` after approval. Omit `dryRunSummary` or use `describe_prompt` only when you need exact full prompt text. In storyboard mode, dry-run summarizes `format`, `animation`, `beat`, `refs`, `cut_plan`, `audio`, and `guardrail` segments plus source/edit paths. HF music-video excludes `beat` by default; pass `contextOverrides.includeShotBeat=true` only when the shot direction should be sent once, or persist `videoPromptSlots.includeShotBeat=true` through `apply_shot_prompts` when future Studio/agent regenerations should keep sending it.
...
- Use `contextOverrides` for one call's input bundle: include an extra guide/ref, exclude a wrong ref, drop the style image, include previous storyboard, change style-note sections, or for video include/drop slots such as `includeShotBeat: true/false` / `includeCutPlan: false`. If a video slot decision should persist for the shot, use `apply_shot_prompts` with `videoPromptSlots` instead. HF music-video excludes the shot beat by default.
...
- Use workflow recipes for repeated production formats. The recipe owns the risky wrapper; the agent fills data/judgment slots such as dialogue, pace, performance, ending, music section, beat timing, or choreography.
...
- Use Codex/Claude native subagents or background workers for bounded off-thread work when it protects the main director context: imagegen repairs, visual triage, continuity checks, prompt-payload audits, or export packaging. Mirage does not create, coordinate, or persist those workers. Name the worker role in the packet (`Image Repair Worker`, `Video Payload Auditor`, `Continuity Auditor`, etc.) and use the packet/receipt contract from the installed Mirage operating docs; workers may inspect, draft, repair local media, or package evidence, but the main director stays responsible for Mirage truth reads, artist approval, paid actions, uploads/imports, locks, and all Mirage state writes.
