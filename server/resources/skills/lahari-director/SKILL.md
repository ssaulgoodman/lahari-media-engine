...
4. Refresh the local notebook. Preferred path: call `mint_cli_token`, then run the returned command for the active shell (`commands.posix` on macOS/Linux, `commands.powershell` on Windows) in the workspace so file bodies do not travel through chat. Fallback path: call `write_project_notebook` and write the returned files manually if shell/npx is unavailable. If the returned `notebookVersion` is newer than the local `lahari/projects/<projectId>/notebook.json` version, refresh before continuing.
