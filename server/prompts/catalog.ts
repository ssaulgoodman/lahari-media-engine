...
- Return one entry per shot.`,
  ...
  },
  {
    ...
    variables: [
      ...
      { name: 'styleImage', description: 'Locked style reference (image input — always sent so the planner can read the medium and adjust output language)' },
      { name: 'prevStoryboardImage', description: 'Optional prev shot storyboard (with image input) when shot.use_prev_storyboard_ref is true' },
...
export const STAGE_META: Record<PromptStage, { label: string; description: string; order: number }> = {
  ...
  utilities:  { label: 'Utilities',  description: 'Gemini utilities, continuity describe, style extract, chat.',        order: 5 },
};
