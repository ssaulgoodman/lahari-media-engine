 ...
 */
const IMAGE_MODELS = ['gemini-3-pro-image-preview', 'gemini-3.1-flash-image-preview'] as const;
...
export const generateImageWithRefs = async (
  ...
  for (const model of IMAGE_MODELS) {
    try {
      ...
      if (images.length === 0) throw new Error(`No image generated by ${model}`);
      if (model !== IMAGE_MODELS[0]) console.log(`[imagen] Fallback to ${model} succeeded`);
      return images[0];
    } catch (err: any) {
      lastError = err;
      ...
      if (String(status) === '503' || String(status) === '429' || msg.includes('UNAVAILABLE') || msg.includes('overloaded') || msg.includes('high demand')) {
        console.warn(`[imagen] ${model} unavailable (${status}), trying fallback...`);
        continue;
      }
