...
export const VIDEO_ACTION_SPECS = {
  ...
  generate_video: {
    ...
    input: {
      ...
      contextOverrides: 'optional per-slot include/exclude and storyboard-video ref controls, e.g. { includeShotBeat: true/false, includeCutPlan: false } or { includeEnvironmentRefs: ["env_uuid_a", "env_uuid_b"] }; preview the effect with dryRun first. HF music-video excludes the shot beat by default; pass includeShotBeat:true only when the shot direction should be sent.',
