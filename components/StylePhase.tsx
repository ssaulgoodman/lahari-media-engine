...
// Per-slot visualize/refine/lock controls live HERE (bespoke), since visualize-style
// and refine-style-direction operate on a specific direction slot rather than at the
// project level — AssetShelf only models tool-level affordances. These tool keys are
// passed to `hideToolKeys` below so they don't double-surface.
...
export const StylePhase: React.FC<Props> = ({
  ...
  const handleRunTool = async (toolKey: string) => {
    if (toolKey === 'brainstorm-style') {
      await handleBrainstorm();
    }
    // visualize-style and refine-style-direction are hidden via
    // hideToolKeys — they fire per-slot via StyleRow's bespoke buttons.
  };
  ...
  // Curated preset grid — shared between unlocked branch and locked Explorer
  // view. Not a registry tool; bespoke artist UX (one-click lock).
            ...
            >
              ...
              <div className="px-4 pb-4">
                ...
                >
                  ...
                  {isLockingPreset ? 'Locking…' : 'Use this style'}
                </button>
              </div>
            </div>
          );
  ...
  // Locked overview block — render when style is locked AND user hasn't
  // explicitly entered the "Explore New" branch. Data-driven.
  if (hasLockedStyle && !showExploredStyles) {
    return (
  ...
  // Explorer view (unlocked OR explicitly "Explore New" while locked).
  // AssetShelf surfaces brainstorm-style. visualize-style + refine-style-direction
  // are hidden — they fire per-slot via StyleRow.
  return (
    <motion.div key="style" {...phaseTransition} className="space-y-6">
      {hasLockedStyle && (
      ...
      {hasLockedStyle && project.styleAssetUrl && (
      ...
      <AssetShelf
        ...
        disabled={isBrainstorming || isLoading}
        hideToolKeys={['visualize-style', 'refine-style-direction']}
      >
        <div className="surface rounded-xl p-4 space-y-3">
          {!hasLockedStyle && (
            <div>
              ...
              <p className="text-zinc-400 text-xs">
                Brainstorm 4 custom visual directions, refine and visualize each, then lock one as the project style.
              </p>
            </div>
          )}
      ...
      {styleSlots.length > 0 && (
        <div className="space-y-3">
          ...
          {/* Custom slot — bespoke "your own description" path with optional upload */}
          <input
            type="file"
            ref={fileInputRef}
            accept="image/*"
      ...
      {!isBrainstorming && styleSlots.length === 0 && !hasLockedStyle && (
        <div className="flex flex-col items-center justify-center h-40 space-y-2 text-zinc-400">
          <p className="text-sm">Hit "Brainstorm style" above to explore visual directions.</p>
          <p className="text-[11px]">Or upload a reference image to skip exploration entirely.</p>
        </div>
      )}
    </motion.div>
  );
