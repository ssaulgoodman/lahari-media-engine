...
export const PromptToolkit: React.FC<PromptToolkitProps> = ({
  ...
  const refineImageRef = React.useRef<HTMLInputElement>(null);
  ...
  const handleRefine = async (feedback: string) => {
    ...
    const img = refineImage?.file;
    onRefineStart(key);
    try {
      if (isFirstFrame) await onRefinePrompt(scene.id, shot.id, feedback, img);
      else if (isEndFrame) await onRefineEndFramePrompt?.(shot.id, feedback, img);
      else if (isVideo) await onRefineVideoPrompt?.(shot.id, feedback, img);
    } finally {
      onRefineEnd(key);
      if (refineImage) { URL.revokeObjectURL(refineImage.previewUrl); setRefineImage(null); }
    }
  };
          ...
          {!shot.locked && (
            <>
              ...
              <div className="flex gap-2">
                ...
                />
                {/* Attach image button */}
                <label className="px-2 py-2 bg-white/[0.04] hover:bg-white/[0.08] text-zinc-400 hover:text-white rounded-md transition-colors flex-shrink-0 self-start cursor-pointer flex items-center" title="Attach a reference image — Claude will see it alongside your feedback">
                  ...
                  <input ref={refineImageRef} type="file" accept="image/*" className="hidden" onChange={e => {
                    const file = e.target.files?.[0];
                    if (file) setRefineImage({ file, previewUrl: URL.createObjectURL(file) });
                    e.target.value = '';
                  }} />
                </label>
                <button
