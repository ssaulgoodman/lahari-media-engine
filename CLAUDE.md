...
Artist director work happens through deployed Mirage remote MCP, not inside this engine repo. `write_project_notebook` materializes an artist workspace with `state/`, editable artifacts, `config/`, `journal.md`, AGENTS/CLAUDE files, and skills under `mirage/projects/<projectId>/`. `state/` is read-only Supabase snapshots. `script.md` is editable; apply it with `run_action(apply_script)` using markdown, which parses the strict markdown format, checks `scriptFingerprint` drift, validates references/durations, and persists through the atomic script apply path.
