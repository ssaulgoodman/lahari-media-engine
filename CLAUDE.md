...
Storyboard action bridge: prefer `run_action` for `apply_storyboard_prompts`, `lock_storyboard`, and `unlock_storyboard`. For paid storyboard generation/refine, use `start_job` with `generate_storyboard`, `bulk_generate_storyboards`, or `refine_storyboard_image` after artist approval.
