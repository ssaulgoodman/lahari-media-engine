...
Timeline editor features currently include media library, video uploads, extra generated clips, split-at-playhead, ripple delete, horizontal scroll, version append, and render history. Timeline drafts are sacred: regenerated shot clips feed the Media Library as new takes instead of rebuilding the artist's saved edit. The render Media Library has a sidebar entry point, badges/new markers for clips not yet in the timeline, upload-to-library for external video clips, create-extra for B-roll/insert clips that do not mutate canonical script shots, soft-hide for unwanted takes, and version append into the timeline. If timeline composition code changes, sync renderer copies with:
