...
- `RENDER_ENGINE` (optional, defaults `ffmpeg`) — engine selection on the Modal renderer. `ffmpeg` (default) uses the fast FFmpeg concat path for eligible timelines (no transitions, no visual effects, all standard cuts) and falls back to Remotion for the rest. `remotion` forces Remotion for everything. See `remotion-renderer/src/ffmpeg-render.ts` → `canRenderWithFfmpeg`. `FFMPEG_PRESET` (default `veryfast`), `FFMPEG_CRF` (default `23`), `FFMPEG_AUDIO_BITRATE` (default `192k`) tune the encode.
