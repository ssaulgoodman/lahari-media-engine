...
comment on column public.lahari_shots.is_extra is
  'Out-of-band insert/B-roll shot. Visible in Studio, but not auto-seeded into the render timeline.';
  ...
  loop
    ...
    loop
      insert into public.lahari_shots (
        ...
        include_prev_cut_plan, excluded_refs, workflow_mode, is_extra,
        sort_order, image_status, video_status
      ) values (
        v_shot->>'id',
        v_scene->>'id',
        ...
        coalesce((v_shot->>'isExtra')::boolean, false),
        v_shot_idx,
        'idle',
        'idle'
      );
