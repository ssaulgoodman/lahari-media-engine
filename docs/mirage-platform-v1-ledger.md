...
| D11 | `music_led.audio = 'skipped'` in WorkflowRecipe. Audio phase tab hidden for music_led | Music video already has audio (the song); no useful UX for v1 |
...
| T3.9 | WorkflowRecipe stages + presets | `server/presets.ts`: add `audio: WorkflowStageState` to `WorkflowRecipe.stages`. Set `music_led.audio = 'skipped'`, `scripted_narrative.audio = 'optional'`. Update `PipelinePreset.audio` field with rules per preset | tsc passes; `getWorkflowRecipe('scripted_narrative').stages.audio === 'optional'` |
...
| ID | Task | Files / Targets | Acceptance |
|---|---|---|---|
| T5.1 | Phase order refactor | `components/AnalysisEditor.tsx` and phase children. Insert Audio phase between Environments and Studio. Hide it entirely when `workflow.stages.audio === 'skipped'` (`music_led`) | tsc passes; music_led shows no Audio tab; scripted_narrative shows it |
