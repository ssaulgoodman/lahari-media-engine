...
2026-05-30 Claude+Codex: Always-on agent surface consolidated. The three always-on instruction surfaces (`AGENTS.md`/`CLAUDE.md`, the MCP `instructions` payload, and the per-skill bodies) were overlapping and self-duplicating. AGENTS.md became the single durable, protocol-agnostic operator base (renamed `AGENTS.md.template` → `AGENTS.template.md`); the `mirage-director` skill was dissolved into it (its role/routing/output-style were always-relevant content trapped in an on-demand file); `HOSTED_MCP_INSTRUCTIONS` was cut ~600→~290 words to a thin starter that hands the file workflow off to AGENTS.md (and is now treated as disposable transport glue, since the durable substrate is the file that survives compaction, not the protocol handshake). Smoke now enforces the split (handoff present, file-workflow absent from the MCP payload, sync guidance owned by AGENTS.md). Commits 86239c1, 02f4c3d.

2026-05-30 Claude+Codex: Response-payload audit. The fat-by-default problem was concentrated in one tool: `open_project` always embedded the full `buildProjectPacket` (uncapped prompt bodies, hashes, neighbors, audio, renders). It now returns the `production` working set by default with `detail='full'` opt-in (reproduces old bytes exactly); `get_project_state` was already lean (`summary`). `list_actions` → lean index pointing at local `config/actions/<surface>.json`; `list_jobs` → status only (dropped per-job result bodies). `detail='full'` is framed in descriptions + AGENTS.md as an off-path debug escape hatch — body needs route to notebook files, never re-inflate the default. Standing rule recorded: over-fetching means a missing compact field to add, not a fatter default. Commits b6a80c1, 92ec9e7, d932cb4.

2026-05-30 Claude+Codex: Skills rebuilt as 8 intuitive node skills (concept-writer, script-writer, art-director, casting-director, sound-director, audio-director, storyboarding, video-director), each teaching the maneuver/repair ladder ("schemas are the buttons, skills are how to play them"), model behavior, and failure modes — not just taste. `continuity-auditor` and `render-triage` were dropped as standalone skills (neither is a graph node) and their content absorbed where the agent actually is. Review then caught a class of bug: skills asserted action behavior that was never read against the code. Four were corrected against source (`applyConcept` marks shot prompts stale and does NOT wipe/fork; `includeConcept` only acts in style-candidate generation; looks generation does not honor `excludeCastRefs`; `bulk_generate_storyboards` is hidden, `materializeForAgent:false`). `.gitignore` was fixed to track `.agents/skills/` (the new copies were silently untracked — `check:notebook` passed locally but a fresh clone would fail). Commits e6baac0, d3162e0.

2026-05-30 Claude+Codex: Two-tier workspace layout. The notebook materialized workspace-level operating files as if the workspace were one project (AGENTS.md carried the project title/id so it churned every open; global action schemas + skill manifest were written under each project folder). Each `NotebookFile` now declares `scope`: workspace-shared files (`AGENTS.md`, `CLAUDE.md`, `.agents/skills`, `.claude/skills`, `config/actions/*`, `config/skills.json`) materialize at the workspace root; project files stay under `mirage/projects/<projectId>/`. AGENTS.md reframed to a multi-project workspace ("operate one project at a time, pick by its folder"). Sync (CLI `@ssaulgoodman420/mirage-cli@0.1.3`) preserves the split: workspace state in root `.mirage-workspace-state.json`, project state per-project, identical shared root files skipped across projects, and old per-project `config/actions`/`config/skills.json` pruned. Commits 9f5d11a (Claude materializer), c21f5f1 (Codex sync). Validation across the day: `npx tsc --noEmit`, `npm run check:notebook` (8 skills), `npm run smoke:agent-contract` (6 checks), `git diff --check`, all green. Session learnings captured in `docs/agent-working-method.md`.
