...
| `bulk_generate_storyboards` | Generates storyboards for many shots in one call | keep `bulk_generate_storyboards` (registry). Use `parallel_run` for already-chosen independent shot actions. | Bulk = declarative server selection of missing/stale/error boards. `parallel_run` = fewer round-trips for explicit shot actions, but still blocks until all finish; true async waits for `start_job`. | ✅ Saul (parallel NOW) |
