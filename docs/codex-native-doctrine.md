...
- **Director sessions** — happen in an artist workspace (any empty folder with the remote MCP installed). Attach to a Lahari project via `/mcp`, materialize the workspace via `mint_cli_token` + `npx @lahari/cli sync` (or `write_project_notebook` fallback), operate through the apply tool surface. The orchestrator skill at `.agents/skills/lahari-director/SKILL.md` (materialized into the artist workspace by notebook sync) drives the protocol.
