...
**Media Library drawer** (`components/MediaLibraryDrawer.tsx`) — bottom-anchored drawer over the timeline canvas. Two layers: uploaded clips row, then horizontal scene picker (S1 S2 ...) → shots in that scene as a horizontal row, each shot showing its active video (ring outline) + older versions as small chips. Click any generated version or uploaded clip → appended to the timeline at the end as a fresh clip (canonical shot data is not modified). Extra inserts/B-roll are not direct random media generators; director agents call `add_extra_shot`, which appends an out-of-band `is_extra` shot under `Extra Shots`, then the normal storyboard/video workflow produces the media-library take. `StepRender` excludes `is_extra` shot videos from automatic timeline seeding, so the editor manually places them. The drawer stays available even before shot videos exist so artists can upload external video clips into the library. Cards show new / in-timeline / added feedback, and unwanted generated/uploaded takes can be soft-hidden from the library. Three-tier thumbnail strategy for generated takes: server-extracted last-frame asset → shot's storyboard or start frame poster (instant paint via `<video poster=...>`) → `#t=0.1` URL-fragment seek with `preload="metadata"`.
