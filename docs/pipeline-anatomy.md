...
**Media Library drawer** (`components/MediaLibraryDrawer.tsx`) — bottom-anchored drawer over the timeline canvas. Two layers: library clips row, then horizontal scene picker (S1 S2 ...) → shots in that scene as a horizontal row, each shot showing its active video (ring outline) + older versions as small chips. Click any generated version, uploaded clip, or extra generated clip → appended to the timeline at the end as a fresh clip (canonical shot data is not modified). The drawer stays available even before shot videos exist so artists can upload external video clips or create B-roll/insert clips into the library. Extra clips are stored as `media_library_video` assets with generated metadata; they do not create scenes/shots or mark prompts stale. Cards show new / in-timeline / added feedback, and unwanted generated/uploaded takes can be soft-hidden from the library. Three-tier thumbnail strategy for generated takes: server-extracted last-frame asset → shot's storyboard or start frame poster (instant paint via `<video poster=...>`) → `#t=0.1` URL-fragment seek with `preload="metadata"`.
