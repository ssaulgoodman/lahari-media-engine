...
**Architecture status (as of 2026-05-23):**
...
All migrations applied and `@ssaulgoodman420/lahari-cli@0.1.0` published. Latest code is pushed to `main`. Railway deploys should use `~/.local/bin/lahari-deploy`, which injects the Lahari project token from `~/.zshrc`; avoid raw `railway up` because it falls back to interactive OAuth when the CLI login expires.
...
- 2026-05-18 — R42 release state updated. Migration `2026-05-17_add_cli_notebook_sync_tokens.sql` applied, `@ssaulgoodman420/lahari-cli@0.1.0` published to npm, and main pushed through `1a7942e`. Railway deploy was initially blocked by a Railway outage / temporarily paused deploys. Future deploys should use `~/.local/bin/lahari-deploy`, not raw `railway up`, so the project-scoped token is used instead of interactive OAuth. Smoke-test `mint_cli_token` plus `npx @ssaulgoodman420/lahari-cli@0.1.0 sync <projectId>` after deploys that touch notebook sync.
...
- 2026-05-23 — Render editor media-boundary hardening documented after live fixes. Timeline drafts are now treated as sacred: regenerated shot videos no longer rebuild saved timeline edits; new clips appear through the render Media Library with sidebar badge/banner and in-library New markers; unwanted non-current takes can be soft-hidden from the library via asset metadata without deleting storage or DB rows. AGENTS.md / CLAUDE.md / Current State updated so future engineering and case-study sessions understand the boundary: generation feeds the library, editing pulls explicitly into the timeline.
